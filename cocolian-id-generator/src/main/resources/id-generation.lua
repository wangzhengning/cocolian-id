local lock_key = 'cocolian-generator-lock'
local sequence_key = 'cocolian-generator-sequence'
local logical_shard_id_key = 'cocolian-generator-logical-shard-id'
...
if redis.call('EXISTS', lock_key) == 1 then
  redis.log(redis.LOG_NOTICE, 'Cocolian: Cannot generate ID, waiting for lock to expire.')
  return redis.error_reply('Cocolian: Cannot generate ID, waiting for lock to expire.')
end
...
if end_sequence >= max_sequence then
  ...
  --]]
  redis.log(redis.LOG_NOTICE, 'Cocolian: Rolling sequence back to the start, locking for 1ms.')
